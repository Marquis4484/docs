...
<Note>
Cloning reads from a source dataset and writes examples into a target dataset, so `datasets:clone` is evaluated twice: once against the source dataset's tags and once against the target dataset's tags. A policy must grant `datasets:clone` on both datasets for the clone to succeed.
</Note>
